...
import { useRoqClient, useClientKFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ClientKInterface, unknown>;

interface ClientKListPageProps {
...
export function ClientKListPage(props: ClientKListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['insurance_provider', 'user', 'appointment.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['gender.contains'],
          ...params.filters,
        },
        'client_k',
      ),
  ...
  const { data, error, isLoading, mutate } = useClientKFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.client_k.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ClientKInterface) => {
    if (hasAccess('client_k', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/client-ks/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.date_of_birth ? format(record?.date_of_birth, 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/client-ks/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('client_k', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/client-ks/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('client_k', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="client_k" />
        </Flex>

        {hasAccess('client_k', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/client-ks/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
